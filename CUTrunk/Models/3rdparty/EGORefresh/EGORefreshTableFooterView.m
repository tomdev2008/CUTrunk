...
//		label.shadowOffset = CGSizeMake(0.0f, 1.0f);
		...
		label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, frame.size.height - 20, self.frame.size.width, 20.0f)];
		label.autoresizingMask = UIViewAutoresizingFlexibleWidth;
		...
		layer.frame = CGRectMake(view.left+1, view.top, 23, 30);
		layer.contentsGravity = kCAGravityResizeAspect;
		layer.contents = (id)[UIImage imageNamed:@"push_down_arrow"].CGImage;
		layer.transform = CATransform3DMakeRotation(M_PI, 0.0f, 0.0f, 1.0f);
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == EGOOPullRefreshLoading) {
		...
		*/
		...
		if (_state == EGOOPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + 46 + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:EGOOPullRefreshNormal];
		} else if (_state == EGOOPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + 46+ RefreshViewHight  && !_loading) {
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + 46+RefreshViewHight && !_loading) {
